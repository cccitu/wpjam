	...
	foreach($menus as $slug => $args){
		...
		if(!$parent){
			...
			if($rendering || $GLOBALS['plugin_page'] == $slug){
				$menu	= wpjam_parse_admin_menu($menu, $slug, '', $rendering);
		...
		if($subs){
			...
			foreach($subs as $sub_slug => $sub_args){
				$sub_slug	= $sub_args['menu_slug'] ?? $sub_slug;

				if($rendering || $GLOBALS['plugin_page'] == $sub_slug){
					$sub_menu	= $object->set_args($sub_args);
					$sub_menu	= wpjam_parse_admin_menu($sub_menu, $sub_slug, $parent, $rendering);
...
function wpjam_parse_admin_menu($menu, $slug, $parent='', $rendering){
	if(is_numeric($slug) || !$menu->menu_title){
		return;
	}
	...
	if($GLOBALS['plugin_page'] == $slug && ($parent || (!$parent && !$menu->subs))){
		...
		$object		= wpjam_register_plugin_page($slug, $menu->get_args());
	}else{
		$object		= null;
	}
...
function wpjam_admin_errors(){
	...
	foreach($errors as $error){
		echo wpjam_wrap_tag($error['message'], 'p')->wrap('div', ['is-dismissible', 'notice', 'notice-'.$error['type']]);
	}
}
...
function wpjam_register_list_table_view($name, $view){
	...
	$view	= wp_parse_args($view, ['type'=>'view', 'show_admin_column'=>true]);
